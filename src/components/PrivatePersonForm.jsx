...
class PrivatePersonForm extends React.Component {
  ...
  extraPersonOptions = (values) => {
    const { options } = this.props.getObject('extraPersons');
    if (values.activePeriod === 'summer') {
      const cottageIndex = options.findIndex(o => o.key === 'cottage');
      options.splice(cottageIndex, 1);
    }
    return options;
  }
  ...
  render() {
    ...
    return (
      <div>
        ...
        <Grid>
          {this.extraPersonOptions(values).map(i =>
            (
              <Grid.Row>
                <Grid.Column width={7}>
                  {i.key === 'cottage' ?
                    <React.Fragment>
                      ...
                      <Form.Group>
                        {i.choices.map((choice, index) =>
                          <Form.Checkbox label={`${choice} ${lan === 'fi' ? 'hlön huone' : 'person room'}`} id={index + 1} checked={values.cottages[index]} onChange={this.props.handleCottageChange} />)}
                      </Form.Group>
                    </React.Fragment>
